...
- **Telegram Bot Message**: The notification payload sent to OpenClaw via Telegram Bot API. Contains the PR URL and potentially diff context.
...
#### Key Business Rules
- **Diff Sanitization Rule**: All diffs must have secrets and sensitive data redacted before being sent to any external service (OpenClaw/Telegram).
...
- **Environment Configuration Rule**: All sensitive tokens (Telegram bot token, GitHub token) must be loaded from environment variables, never hardcoded.

## Strategic Approach

#### Solution Direction
- **Architecture Pattern**: Event-driven pipeline using GitHub Actions as the orchestration layer. The flow is: GitHub PR Event → GitHub Actions Workflow → Fetch Diff (Node.js script with Octokit) → Sanitize Diff → Send to OpenClaw via Telegram Bot API → OpenClaw Agent reviews and responds in Telegram.
- **Technology Stack**: Node.js (CommonJS) with `@octokit/rest` for GitHub API access and `dotenv` for configuration. GitHub Actions for workflow orchestration. Telegram Bot API for OpenClaw integration.
- **General Data Flow**:
...
- **OpenClaw Integration Method**: Use Telegram Bot API to send message to OpenClaw (rather than direct HTTP API) → **Recommended**: Leverages existing OpenClaw Telegram integration. Trade-off: Indirect integration adds a hop; requires Telegram bot token and chat ID configuration.
...
- **Webhook to OpenClaw**: Send diff via HTTP webhook directly to OpenClaw instead of Telegram → **Rejected**: OpenClaw is already configured to receive messages via Telegram Bot API; changing this would require OpenClaw reconfiguration.
...
- **Telegram Bot Credentials**: `TELEGRAM_BOT_TOKEN` and `OPENCLAW_CHAT_ID` are required but the source (user-provided, pre-existing) is not specified. → **Needs clarification**: Are these already available from the existing OpenClaw setup?
...
| 4 | Send message to OpenClaw Telegram bot via Telegram Bot API | Yes | Message format and OpenClaw chat ID need clarification |
